...
explore: product_master {
  ...
  join: sales_data {
    type: left_outer
    relationship: one_to_many
    sql_on: ${sales_data.product_id}=${product_master.product_id} ;;
  }
  join: category_master {
    type: left_outer
    relationship: many_to_one
    sql_on: ${category_master.category_id}=${product_master.category_id}_id} ;;
  }
}
explore: member_info {
  join: sales_data {
    type: left_outer
    relationship: one_to_many
    sql_on: ${sales_data.customer_id}=${member_info.customer_id} ;;
  }
}
